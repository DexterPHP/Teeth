...
@section('title', 'حركات محاسبة |  اختيار طبيب   ')
@if(isset($center) and isset($DoctordsData))
@section('content_header')
    <h1 class="text-center">     حركات الايداع في صندوق طبيب</h1>
    <h3 class="text-center">الرجاء اختيار الطبيب في
    ...
    <table id="example" class="table table-striped table-bordered display text-center" style="width:100%">
        ...
        @if(isset($admin))
            @foreach($DoctordsData as $center)
                <tr>
                    <th><a href="push/{{ $center->uuid }}" title="سحب"><i class="fas fa-id-card"></i></a></th>
        ...
        @else
            <tr>
                <th><a href="push/{{ $DoctordsData->uuid }}" title="سحب"><i class="fas fa-id-card"></i></a></th>
