...
@section('title', 'سحب مبلغ من صندوق المركز')

@if(isset($center))
@section('content_header')
    <h1 class="text-center">سحب مبلغ في الصندوق  </h1>
    ...
    <form method="post">
        ...
        @if(session('Greate'))
            <div class="alert alert-success alert-dismissible text-right">
                ...
                لقد عماية سحب  المبلغ بشكل سليم
            </div>
        @endif
        ...
        @endif
        @if(session('StopLessThan'))
            <div class="alert alert-danger alert-dismissible text-right">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                <h5><i class="icon fas fa-check"></i> توقف </h5>
               انت تحاول سحب مبلغ اكبر من الموجود في الصندوق
            </div>
        @endif
